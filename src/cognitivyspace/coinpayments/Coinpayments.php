...
use CognitivySpace\Coinpayments\Exceptions\JsonParseException;
use CognitivySpace\Coinpayments\Exceptions\MessageSendException;
...
class Coinpayments
{
    ...
    {
        $acceptableFields = ['address', 'buyer_email', 'buyer_name', 'item_name', 'item_number', 'invoice', 'custom', 'ipn_url'];
    ...
    {
        $req = collect($withdrawals)
            ->flatMap(function ($withdrawal, $index) {
                ...
                return collect($withdrawal)
                    ->flatMap(function ($value, $key) use ($index) {
