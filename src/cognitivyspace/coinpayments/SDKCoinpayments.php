...
namespace CognitivySpace\Coinpayments;

use Illuminate\Http\Request;
use CognitivySpace\Coinpayments\Receipt;
use CognitivySpace\Coinpayments\Models\Ipn;
use CognitivySpace\Coinpayments\Models\Log;
use CognitivySpace\Coinpayments\Coinpayments;
use CognitivySpace\Coinpayments\Enums\IpnType;
use CognitivySpace\Coinpayments\Models\Deposit;
use CognitivySpace\Coinpayments\Models\Transfer;
use CognitivySpace\Coinpayments\Models\Conversion;
use CognitivySpace\Coinpayments\Models\Withdrawal;
use CognitivySpace\Coinpayments\Models\Transaction;
use CognitivySpace\Coinpayments\Models\MassWithdrawal;
use CognitivySpace\Coinpayments\Models\CallbackAddress;
use CognitivySpace\Coinpayments\Enums\CoinpaymentsCommand;
use CognitivySpace\Coinpayments\Exceptions\CoinPaymentsException;
use CognitivySpace\Coinpayments\Exceptions\IpnIncompleteException;
use CognitivySpace\Coinpayments\Exceptions\CoinPaymentsResponseError;

/**
 * Class SDKCoinpayments
 *
 * @package CognitivySpace\Coinpayments
 ...
 *
 */
class SDKCoinpayments extends Coinpayments
{
    private $app;
    ...
    public function __construct($app)
    {
        ...
        parent::__construct(cp_conf('private_key'), cp_conf('public_key'), cp_conf('merchant_id'), cp_conf('ipn_secret'), cp_conf('ipn_url'), cp_conf('format'));
    }
    ...
    private function registerMassWithdrawal(Receipt $receipt)
    {
        ...
        $mass_withdrawal->withdrawals = collect($receipt->getResponse()['result'])->flatMap(function ($value, $wdIndex) use ($mass_withdrawal, $requests) {
